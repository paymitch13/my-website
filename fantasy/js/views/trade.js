...
import { evaluateTrade, buildEntries, suggestBalancers } from '../trade.js';
...
function renderResult(app, result, repaint) {
    ...
    if (loser && winner && result.verdict.gap > 0.08) {
        const gap = Math.max(0, winner.valueNet);
        const picks = suggestBalancers({
            cfg: app.league.cfg,
            ctx: app.ctx,
            rankings: app.rankings,
            fromTeam: winner.team,
            gap,
            exclude: winner.side.sending,
            limit: 4,
        });
        if (picks.length) {
            wrap.append(el('div', { class: 'section-head' }, el('h2', {}, 'How to even it out')));
            wrap.append(
                el(
                    'div',
                    { class: 'card' },
                    el(
                        'p',
                        { class: 'muted small' },
                        `${loser.team.name} is short about ${round(gap, 0)} points of value. Asking ${winner.team.name} to add one of these gets the ledger close:`
                    ),
                    el(
                        'div',
                        { class: 'pick-list' },
                        ...picks.map((p) =>
                            el(
                                'div',
                                { class: 'pick', style: 'cursor:default' },
                                playerCell(p.player, { rank: p.posRank }),
                                el('div', { class: 'grow' }),
                                el('span', { class: 'num small muted' }, `${round(p.value, 0)} value`),
                                p.overshoot
                                    ? tag('too much', 'warn')
                                    : tag(`closes ~${p.closes}%`, p.closes >= 80 ? 'good' : '')
                            )
                        )
                    )
                )
