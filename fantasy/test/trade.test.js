...
import { evaluateTrade, buildEntries, suggestBalancers } from '../js/trade.js';
...
test('balancer suggestions close the gap without wild overshoot', () => {
    const rankings = new Map();
    const teams = league(rankings, [{ name: 'A', players: BALANCED }]);
    const entries = buildEntries(teams[0].players, rankings, ctx);
    const target = entries[3].value;
    const picks = suggestBalancers({ cfg, ctx, rankings, fromTeam: teams[0], gap: target, limit: 3 });
    assert.ok(picks.length > 0);
    assert.ok(Math.abs(picks[0].value - target) <= Math.abs(picks[picks.length - 1].value - target));
});
