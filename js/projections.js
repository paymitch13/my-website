...
/**
 * Weekly rows keep the schedule fields the season endpoint lacks -- opponent,
 * team and game id -- which is what makes matchup and weather lookups possible.
 */
export function normalizeWeekly(rows) {
    const out = {};
    for (const row of rows || []) {
        const id = row?.player_id;
        const stats = row?.stats;
        if (!id || !stats) continue;
        if (stats.pts_half_ppr === undefined && stats.pts_std === undefined && stats.pts_ppr === undefined) continue;
        ...
        out[id] = {
            id,
            pos: row.player?.position || (row.player?.fantasy_positions || [])[0] || null,
            team: row.team || null,
