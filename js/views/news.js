...
export default function renderNews(app) {
    ...
    async function paintFeed() {
        ...
        // Trades get their own log: they are the events people actually argue
        // about, and burying them among waiver adds hides them.
        const trades = tradesOnly(app.transactions);
        wrap.append(
            el(
                'div',
                { class: 'section-head' },
                el('h2', {}, 'Trade log'),
                el('span', { class: 'hint grow' }, trades.length ? `${trades.length} this season` : 'updates automatically when a trade is accepted')
            )
        );
        wrap.append(
            trades.length
                      ...
                      ...trades.map((t) =>
                          el(
                              ...
                              el(
                                  ...
                                  el(
                                      'div',
                                      { class: 'trade-log-lines' },
                                      ...t.sides.map((side) =>
                                          el(
                                              'div',
                                              { class: 'small' },
                                              el('span', { class: 'dim' }, `${side.name} gets: `),
                                              side.gets.length
                                                  ? el(
                                                        'span',
                                                        {},
                                                        ...side.gets.flatMap((pl, i) => [
                                                            i ? ', ' : '',
                                                            playerLink(pl),
                                                        ])
                                                    )
                                                  : el('span', { class: 'dim' }, 'picks or cash only')
                                          )
                                      ),
                                      ...t.picks.map((pk) =>
                                          el('div', { class: 'tiny dim' }, `${pk.season} round ${pk.round} pick changes hands`)
                                      )
                                  ),
                                  el(
                                      'div',
                ...
                : el(
                      'div',
                      { class: 'card' },
                      el('p', { class: 'muted' }, 'No trades in this league yet this season. When one is accepted in Sleeper it will appear here within a couple of minutes.')
                  )
        );
